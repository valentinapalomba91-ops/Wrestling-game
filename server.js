...
function nextTurnLogic() {
    ...
    while (gameState.players[nextIndex].skippedTurns > 0) {
        ...
        if (nextIndex === startIndex) {
            ...
            if(turnsSkipped) break; // Usciamo dopo aver controllato un ciclo completo
        }
    }
...
function processPlayerMove(diceRoll, isCardMove = false) {
    ...
    // Ritorna errore se il giocatore non è trovato (prevenzione lato server)
    if (!player) {
        return {
            playerId: null,
            diceRoll: diceRoll,
...
function processCardEffect(card) {
    ...
    return {
        ...
        // ✅ CORREZIONE: Invia l'ID del prossimo giocatore dopo la logica di passaggio turno/extra turn
    ...
    socket.on('disconnect', () => {
        ...
        if (playerIndex !== -1) {
             ...
             if (gameState.players.length > 0) { 
                 ...
                 if (wasCurrent) {
                    nextTurnLogic(); // Trova il prossimo giocatore valido
                 }
                 
             } else {
                 gameState.game_over = true;
                gameState.currentTurnIndex = 0; // Reset per sicurezza
             }
